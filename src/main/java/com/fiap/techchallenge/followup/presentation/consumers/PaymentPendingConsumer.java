...
public class PaymentPendingConsumer {
    ...
    @KafkaListener(topics = "${kafka.topic.payment-pending}", groupId = "${spring.kafka.consumer.group-id}")
    public void consume(@Payload String message) throws JsonProcessingException {
        try {
            ...
            OrderUpdateStatusResquestDto orderUpdateStatusResquestDto = OrderUpdateStatusResquestDto.builder()
                    ...
                    .status(StatusEnum.PAYMENT_PENDING.name().toLowerCase())
                    .build();
            ...
            ErrorConsumerDto<Object> errorConsumer = ErrorConsumerDto.builder()
                    ...
                    .rawData(message)
                    .build();
            asynchronousRequestPort.sendPaymentPendingDl(errorConsumer);
        } catch (Exception e) {
